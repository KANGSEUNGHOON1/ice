<template>
  <div class="space-y-6 text-gray-800 dark:bg-black dark:text-gray-200 p-4 rounded">
    <!-- 페이지 헤더 -->
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">예약 관리</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 md:p-6 hover:shadow-lg transition-shadow">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-xl md:text-2xl font-semibold text-gray-800 dark:text-white">24건</p>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 md:p-6 hover:shadow-lg transition-shadow">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">확정 예약</h3>
            <p class="text-xl md:text-2xl font-semibold text-gray-800 dark:text-white">18건</p>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 md:p-6 hover:shadow-lg transition-shadow">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">대기 예약</h3>
            <p class="text-xl md:text-2xl font-semibold text-gray-800 dark:text-white">6건</p>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
      <div class="p-4 border-b border-gray-200 dark:border-gray-700">
        <h2 class="text-lg font-semibold text-gray-800 dark:text-white">예약 목록</h2>
      </div>
              ...
              <div>
                ...
                <div class="space-y-2">
                  ...
                  <div class="flex items-center">
                    ...
                    <span class="text-sm text-gray-900 dark:text-white">{{ selectedReservation.user }}</span>
                  </div>
                  <div class="flex items-center">
                ...
                <div class="space-y-2">
                  ...
                  <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                      <div class="relative" v-for="(photo, index) in selectedReservation.photos" :key="index">
                        <img :src="photo" class="w-full h-32 object-cover rounded-md" />
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div class="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] overflow-y-auto" @click.stop>
        <div class="p-6 border-b border-gray-200">
          <div class="flex justify-between items-center">
            ...
            <button class="text-gray-400 hover:text-gray-500" @click="closeTechnicianSearchModal">
              <i class="fas fa-times"></i>
            </button>
          </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">기사명</th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">구분</th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    휴대전화
                  </th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">정산율</th>
                  <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    활동지역
                  </th>
              ...
              <tbody v-for="tech in paginatedTechnicians" :key="tech.id" class="bg-white divide-y divide-gray-200">
                <tr class="hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ tech.settlementRate }}%</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {{ getAreaText(tech.area) }}
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button @click="selectTechnician(tech)" class="text-indigo-600 hover:text-indigo-900">선택</button>
                  </td>
                </tr>
              </tbody>
            </table>
...
const filteredReservations = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    result = result.filter(
      (r) => r.user.toLowerCase().includes(query) || r.id.toString().includes(query) //숫자를 문자열로 바꿈
    );
  }
  // 상태필터링
...
const showReservationDetails = (reservation) => {
  selectedReservation.value = {
    ...
    photos: reservation.photos || ['/images/ice_cream.jpg', '/images/ice_cream2.jpg', '/images/ice_cream3.png'], // 요구사항이 없으면 "요구사항 없음"으로 설정
...
const filteredTechnicians = computed(() => {
  return technicians.value.filter((tech) => {
    ...
    const matchesType = technicianSearchFilter.value.type === 'all' || tech.type === technicianSearchFilter.value.type;
    //정산비율이(settlementRate) "all"이면 모두 포함, 아니면 선택한 비율과 같은 기사만 포함
    const matchesRate =
      technicianSearchFilter.value.settlementRate === 'all' ||
      tech.settlementRate.toString() === technicianSearchFilter.value.settlementRate;
    // 지역에 (area)"all"이면 모두 포삼, 아니면 선택한 지역과 같은 기사만 포함
    const matchesArea = technicianSearchFilter.value.area === 'all' || tech.area === technicianSearchFilter.value.area;
