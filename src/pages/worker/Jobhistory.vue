<template>
  <div class="space-y-4 md:space-y-6 p-4 md:p-6 rounded-lg bg-slate-200 shadow-sm">
    <!-- 페이지 헤더 -->
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div>
        <h1 class="text-xl md:text-2xl font-bold text-gray-800">작업 내역</h1>
        <p class="text-sm text-gray-500 mt-1">완료된 작업의 내역과 평가를 확인할 수 있습니다.</p>
      </div>
            ...
            <div class="flex gap-1 mt-1">
              <template v-for="n in 5" :key="n">
                ...
                <i
                  ...
                  class="fas fa-star-half-alt text-sm md:text-base text-yellow-400"
                ></i>
      ...
      <div class="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-200 p-4 md:p-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-xl md:text-2xl font-semibold text-gray-800">400,000원</p>
          </div>
        </div>
      </div>
    </div>
        ...
        >
          ...
          <div class="flex items-center gap-2">
            <div class="flex gap-1">
              <template v-for="n in 5" :key="n">
                <i v-if="n <= Math.floor(job.rating)" class="fas fa-star text-sm md:text-base text-yellow-400"></i>
                <i
            ...
            <span v-if="job.review" class="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">작업 시간</th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">수령 금액</th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">평가</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-for="job in filteredJobs" :key="job.id" class="hover:bg-gray-50 transition-colors duration-200">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-medium text-gray-900">
                  {{ formatDate(job.date) }}
                </div>
                <div class="text-sm text-gray-500">{{ getDayOfWeek(job.date) }}요일</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <i class="fas fa-user-circle text-indigo-500 mr-2"></i>
                  <span class="text-sm font-medium text-gray-900">{{ job.customer }}</span>
                </div>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <i class="fas fa-clock text-gray-400 mr-2"></i>
                  <span class="text-sm text-gray-500">{{ job.duration }}분</span>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <i class="fas fa-won-sign text-gray-400 mr-2"></i>
                  <span class="text-sm text-gray-500">{{ job.earnings.toLocaleString() }}원</span>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center gap-2">
                  <div class="flex gap-1">
                    <template v-for="n in 5" :key="n">
                      <i
                        ...
                        class="fas fa-star text-sm md:text-base text-yellow-400"
                      ></i>
                      <i
                        v-else-if="n - 0.5 <= job.rating"
                        class="fas fa-star-half-alt text-sm md:text-base text-yellow-400"
    ...
    >
      <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" @click.stop>
        <div class="p-6 border-b border-gray-200 flex justify-between items-center">
          ...
          <button @click="selectedJob = null" class="text-gray-400 hover:text-gray-500 transition-colors duration-200">
        ...
        <div class="p-6 space-y-6">
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label class="block text-sm text-gray-500 mb-1">작업 날짜</label>
                <p class="text-gray-900">{{ formatDate(selectedJob.date) }} ({{ getDayOfWeek(selectedJob.date) }})</p>
              </div>
              ...
              <div>
                <label class="block text-sm text-gray-500 mb-1">수령 금액</label>
                <p class="text-gray-900">{{ selectedJob.earnings.toLocaleString() }}원</p>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-2">
              <div v-for="task in selectedJob.cleaningTasks" :key="task" class="flex items-center gap-2">
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-4">
              ...
              <div v-if="selectedJob.review">
                <label class="block text-sm text-gray-500 mb-1">리뷰 내용</label>
...
const jobHistory = ref([
  {
    ...
    rating: 3.5,
    status: '완료',
    review: '매우 친절하고 전문적인 서비스였습니다. 다음에도 꼭 부탁드리고 싶어요!',
  ...
  },
  {
    ...
    status: '완료',
    review: null,
    cleaningTasks: ['거실 청소', '주방 청소', '화장실 청소', '창문 청소', '베란다 청소'],
  },
  {
    id: 4,
    customer: '이지은',
    ...
    cleaningTasks: ['거실 청소', '주방 청소', '화장실 청소', '창문 청소', '베란다 청소', '냉장고 청소'],
  },
  {
    id: 5,
    customer: '정민호',
...
const filteredJobs = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    result = result.filter(
      (job) => job.customer.toLowerCase().includes(query) || job.address.toLowerCase().includes(query)
    );
  }
  return result;
});
